...
Public Class NavControler
	...
	Public Shared Sub NavTo(ByVal Action As String)
		...
		' if it exists
		HttpContext.Current.Response.Redirect(BaseHelper.FQDNBasePage & "?Action=" & Action)
	End Sub	'NavTo
	...
	Public Shared Sub NavTo(ByVal Action As String, ByVal ThePage As Page, ByVal PlaceHolder As PlaceHolder)
		...
		Try
			...
			Else
				...
				theURL &= BaseHelper.GetURL()
				HttpContext.Current.Response.Redirect(BaseHelper.RootSite & moduleProfileInfo.Source & theURL)
			End If
	...
	Public Shared Sub NavTo(ByRef ex As Exception)
		Dim myURL As String = "~/" & BaseHelper.FQDNBasePage & "?Action=error"
		Dim myPath As String = HttpContext.Current.Request.Path
		If InStr(myPath, BaseHelper.AppPath) <> 0 Then
			myPath = Mid(myPath, Len(BaseHelper.AppPath) + 2, Len(myPath))
		End If
		myURL = BaseHelper.FQDNBasePage & "?Action=error&ReturnURL=" & HttpContext.Current.Request.QueryString("Action") & BaseHelper.GetURL
	...
	Private Shared Sub NavTo(ByVal WorkFlowProfileInfo As MWorkFlowProfileInfo, Optional ByVal URL As String = "")
		...
		HttpContext.Current.Response.Redirect(BaseHelper.FQDNBasePage & "?Action=" & myModuleProfileInfo.Action & URL)
	End Sub
	...
	Public Shared Sub BuildModuleList(ByVal parent As Control, ByVal myModuleProfileInfo As MModuleProfileInfo, ByRef ThePage As Page)
		...
		For i = 0 To 0
			...
			If ((moduleSource <> "") And (moduleSource <> "System.DBNull")) Then
				Try
					If ((Not BaseHelper.AlwaysLeftNav) And parent.ID.Trim.ToLower.IndexOf("right") > -1 And HttpContext.Current.Request.QueryString("Action").ToLower.Trim = "logon") Then
					...
					UISpecificModuleSourcePath = UISpecificModuleSource.Replace("Modules\", BaseHelper.UIPath() & BaseHelper.UI(ClientChoicesUtility.GetSelectedBusinessUnit) & "\UserControls\")
					...
					Else
						...
						UISpecificModuleSource = UISpecificModuleSource.Replace("Modules\", "UI\" & BaseHelper.UI(ClientChoicesUtility.GetSelectedBusinessUnit) & "\UserControls\")
	...
	Public Shared Sub StartWorkFlow(ByVal WorkFlowName As String, Optional ByVal URL As String = "")
		...
		Dim myWorkFlowProfileCollection As MWorkFlowProfileInfoCollection
	...
	Public Shared Sub [Next]()
		...
		If Not nextWorkFlowProfile Is Nothing Then
			...
			NavControler.NavTo(myModuleProfileInfo.Action & BaseHelper.GetURL)
		Else
	...
	Public Shared Sub Previous()
		...
		Else
			...
			NavTo(myModuleProfileInfo.Action & BaseHelper.GetURL)
		End If
	End Sub
