...
#Region "Private Methods"
        ''' <summary>
        ''' Formats a an error message containing the stored procedure name and the MySqlParameter/values.
        ''' </summary>
        ...
        Private Function formatError(ByVal parameters() As MySqlParameter, ByVal storedProcedure As String, ByVal yourExMSG As String) As String
            Dim mMessage As String = vbCrLf + "Error executing '" + storedProcedure + "' :: " + vbCrLf
            Dim testParameter As MySqlParameter
...
#Region "IDDBInteraction Members"
        ...
        Protected Overridable Sub ExecuteNonQuery(ByVal storedProcedure As String, ByVal parameters() As MySqlParameter)
            ...
            Dim dbCommand As MySqlCommand = Nothing

            Try
                db = New MySqlConnection(ConnectionString)
                dbCommand = New MySqlCommand(storedProcedure, db)
                dbCommand.CommandTimeout = getCommandTimeOut()
                If Not (parameters Is Nothing) Then
                    If parameters.Length > 0 Then
                        For Each myParameter_loopVariable As MySqlParameter In parameters
        ...
        Protected Overridable Function GetDataSet(ByVal storedProcedure As String, ByVal parameters() As MySqlParameter) As DataSet
            ...
            Dim mParameter As MySqlParameter = Nothing
            Dim db As MySqlConnection = Nothing
            Dim dbCommand As MySqlCommand = Nothing
            Dim mAdapter As MySqlDataAdapter = Nothing
            Try
                db = New MySqlConnection(ConnectionString)
                dbCommand = New MySqlCommand(storedProcedure, db)
                If Not parameters Is Nothing Then
                ...
                mAdapter.SelectCommand = dbCommand
                mAdapter.Fill(mRetDataSet)
                ...
                If Not parameters Is Nothing Then
                    If parameters.Length > 0 Then
                        ...
                        For Each mParameter In parameters
                            dbCommand.Parameters.Add(mParameter)
                        Next
                        mReader = dbCommand.ExecuteReader()
                    Else
                        mReader = dbCommand.ExecuteReader()
                    End If
                Else
        ...
        Protected Overridable Function GetDataRow(ByVal storedProcedure As String) As DataRow
            Dim mParameters As MySqlParameter() = Nothing
        ...
        ''' <summary>
        ''' Retruns a MySqlParameter given the ParameterName, ParameterValue and Direction.
        ''' </summary>
