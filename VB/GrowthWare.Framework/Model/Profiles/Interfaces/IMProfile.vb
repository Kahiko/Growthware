Namespace Model.Profiles.Interfaces
    ''' <summary>
    ''' Ensures the basic properties are avalible to all Profile model objects.
    ''' </summary>
    ''' <remarks>
    ''' If it is decided to use entities in the future then
    ''' this interface should be used for the save, delete, and getitem methods.
    '''  </remarks>
    Public Interface IMProfile
        ''' <summary>
        ''' Account ID used to add
        ''' </summary>
        Property AddedBy As Integer

        ''' <summary>
        ''' Date the row was added.
        ''' </summary>
        Property AddedDate As Date

        ''' <summary>
        ''' Unique numeric identifier
        ''' </summary>
        Property Id As Integer

        ''' <summary>
        ''' Column name for the Id property.
        ''' </summary>
        Property IdColumnName As String

        ''' <summary>
        ''' String representation normaly unique
        ''' </summary>
        Property Name As String

        ''' <summary>
        ''' Column name for the Name property.
        ''' </summary>
        Property NameColumnName As String

        ''' <summary>
        ''' Account ID used to update
        ''' </summary>
        Property UpdatedBy As Integer

        ''' <summary>
        ''' The date lasted updated
        ''' </summary>
        Property UpdatedDate As Date
    End Interface
End Namespace
