...
#Region " PRIVATE METHODS"
    ...
    Private Function PopulateGroupInfoFromSqlDataReader(ByVal dr As SqlDataReader) As MGroupInfo
...
#End Region
    ...
    Public Function AddGroup(ByRef GroupInfo As MGroupInfo, Optional ByVal Account_Seq_id As Integer = 1) As Integer Implements IGroups.AddGroup
        ...
        With GroupInfo
            SqlHelper.ExecuteNonQuery(ConnectionString, CommandType.StoredProcedure, "ZBP_ADD_GROUP_INFO", New SqlParameter() { _
            New SqlParameter("@P_BUSINESS_UNIT_SEQ_ID", .BusinessUnitId), _
            New SqlParameter("@P_GROUP_NAME", .GroupName), _
            New SqlParameter("@P_GROUP_DESCRIPTION", .GroupDescription), _
    ...
    Public Function GetRolesByBusinessUnit(ByVal GroupId As Integer, ByVal BusinessUnitSeqId As Integer) As String() Implements IGroups.GetRolesByBusinessUnit
        ...
        Try
            reader = SqlHelper.ExecuteReader( _
               ...
               "ZBP_GET_RLS_FOR_GROUP_BY_BU", _
               New SqlParameter() { _
             New SqlParameter("@P_GROUP_SEQ_ID", GroupId), _
             New SqlParameter("@P_BUSINESS_UNIT_SEQ_ID", BusinessUnitSeqId) _
               } _
               )
            While reader.Read()
    ...
    Public Function SearchGroups(ByVal GroupName As String, ByVal BusinessUnitSeqId As Integer) As System.Data.DataSet Implements IGroups.SearchGroups
        ...
        Try
            dsGroups = SqlHelper.ExecuteDataset( _
             ConnectionString, _
             CommandType.StoredProcedure, _
             "ZBP_GET_ALL_GRPS_BY_BU", New SqlParameter() _
             {New SqlParameter("@P_GROUP_NAME", GroupName), _
             New SqlParameter("@P_BUSINESS_UNIT_SEQ_ID", BusinessUnitSeqId)} _
             )
        Catch ex As Exception
    ...
    Public Function UpdateAGroup(ByVal GroupInfo As MGroupInfo, Optional ByVal Account_Seq_id As Integer = 1) As Boolean Implements IGroups.UpdateAGroup
        ...
        returnParam.Direction = ParameterDirection.ReturnValue
        With GroupInfo
            SqlHelper.ExecuteNonQuery(ConnectionString, CommandType.StoredProcedure, "ZBP_UPDATE_GROUP_INFO", New SqlParameter() { _
            New SqlParameter("@P_GROUP_SEQ_ID", .GroupId), _
            New SqlParameter("@P_GROUP_NAME", .GroupName), _
            New SqlParameter("@P_GROUP_DESCRIPTION", .GroupDescription), returnParam, _
            New SqlParameter("@P_ADDUPD_BY", Account_Seq_id) _
            })
        End With
        retVal = CInt(returnParam.Value)
    ...
    Public Sub UpdateRoles(ByVal GroupId As Integer, ByVal BusinessUnitSeqId As Integer, ByVal roles() As String, Optional ByVal Account_Seq_id As Integer = 1) Implements IGroups.UpdateRoles
        ...
        Try
            ...
            SqlHelper.ExecuteNonQuery(ConnectionString, _
             CommandType.StoredProcedure, _
             "ZBP_UPDATE_GROUP_RLS", _
             New SqlParameter() { _
             New SqlParameter("@P_GROUP_SEQ_ID", GroupId), _
             New SqlParameter("@P_BUSINESS_UNIT_SEQ_ID", BusinessUnitSeqId), _
             New SqlParameter("@P_ROLES", rolesCommaSeparated), _
             New SqlParameter("@P_ADDUPD_BY", Account_Seq_id) _
             })
    ...
    Public Function GetGroupInfo(ByVal GroupId As Integer) As MGroupInfo Implements IGroups.GetGroupInfo
		Dim profile As MGroupInfo = Nothing
		Dim reader As SqlDataReader = Nothing
        Try
            reader = SqlHelper.ExecuteReader( _
               ConnectionString, _
               CommandType.StoredProcedure, _
               "ZBP_GET_GROUP_INFO", _
               New SqlParameter() {New SqlParameter("@P_GROUP_ID", GroupId)} _
               )
            If reader.Read() Then
                profile = PopulateGroupInfoFromSqlDataReader(reader)
            End If
        Catch ex As Exception
            Throw ex
        Finally
    ...
    Public Sub DeleteGroup(ByVal GroupSeqId As String, ByVal BusinessUnitSeqId As Integer, Optional ByVal Account_Seq_Id As Integer = 1) Implements IGroups.DeleteGroup
        Try
            SqlHelper.ExecuteNonQuery(ConnectionString, CommandType.StoredProcedure, _
                "ZBP_DEL_GROUP", New SqlParameter() { _
            New SqlParameter("@P_GROUP_SEQ_ID", GroupSeqId), _
            New SqlParameter("@P_BUSINESS_UNIT_SEQ_ID", BusinessUnitSeqId), _
            New SqlParameter("@P_ADDUPD_BY", Account_Seq_Id) _
            })
        Catch ex As Exception
