		...
		Private Sub acquireRequestState(ByVal sender As Object, ByVal e As EventArgs)
			...
			If HttpContext.Current.Request.Path.ToUpper(New CultureInfo("en-US", False)).IndexOf(".ASPX", StringComparison.Ordinal) > -1 Then
				If Not HttpContext.Current.Request.QueryString("Action") Is Nothing Then
					Dim mAction As String = HttpContext.Current.Request.QueryString("Action").ToString()
					' process security
					...
					Dim mFunctionUtilty As FunctionUtility = New FunctionUtility()
					Dim mFunctionProfile As MFunctionProfile = mFunctionUtilty.GetFunction(mAction)
					Dim mSecurityInfo As MSecurityInfo = New MSecurityInfo(mFunctionProfile, mAccountProfile.AssignedRoles)
				Else
