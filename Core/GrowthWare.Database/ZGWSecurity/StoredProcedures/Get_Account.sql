...
BEGIN
	IF @P_Is_System_Admin = 1
		BEGIN
			...
			SELECT AccountSeqId AS ACCT_SEQ_ID
				, Account AS ACCT
				, Email
	...
	BEGIN
		IF @P_Debug = 1
			PRINT 'Selecting all accounts for Entity ' + CONVERT(VARCHAR(MAX), @P_SecurityEntitySeqId)
		...
		INSERT INTO @V_Accounts
			...
			WHERE Roles_Security_Entities_Accounts.AccountSeqId = Accounts.AccountSeqId
				AND Roles_Security_Entities_Accounts.RolesSecurityEntitiesSeqId = Roles_Security_Entities.RolesSecurityEntitiesSeqId
				AND Roles_Security_Entities.SecurityEntitySeqId IN (
		...
		UNION
			...
			FROM ZGWSecurity.Accounts AS Accounts WITH (NOLOCK)
				...
				,ZGWSecurity.Roles_Security_Entities WITH (NOLOCK)
				,ZGWSecurity.Roles WITH (NOLOCK)
			WHERE ZGWSecurity.Groups_Security_Entities_Accounts.AccountSeqId = Accounts.AccountSeqId
