...
#region Member Fields
		...
		private static byte[] KEY_64 = { 83, 68, 91, 37, 128, 64, 92, 197 };

		private static byte[] IV_64 = { 6, 55, 118, 219, 92, 197, 78, 69 };
		//24 byte or 192 bit key for TripleDES
		private static byte[] KEY_192 = {83, 68, 91, 37, 128, 64, 92, 197,
		  87, 215, 61, 243, 148, 20, 252, 34,
		  38, 69, 83, 201, 74, 211, 6, 98};
		//24 byte or 192 bit Initialization Vector for TripleDES
		private static byte[] IV_192 = {6, 55, 118, 219, 92, 197, 78, 69,
...
#region Public Methods
		...
		public static string Encrypt(string valueToEncrypt, EncryptionTypes encryptionType)
		{
		...
		/// <returns>Encrypted string</returns>
		/// <remarks>EncryptionType is case sensitive.</remarks>
		public static string Encrypt(string valueToEncrypt, EncryptionTypes encryptionType, string saltExpression)
		{
		...
		/// <remarks></remarks>
		public static string Decrypt(string valueToDecrypt, EncryptionTypes encryptionType)
		{
			switch (encryptionType) {
				case EncryptionTypes.TripleDES:
		...
		/// <returns>Decrypted string</returns>
		/// <remarks></remarks>
		public static string Decrypt(string ValueToDecrypt, EncryptionTypes EncryptionType, string SaltExpression)
		{
			setKeys = SaltExpression;
			switch (EncryptionType) {
				case EncryptionTypes.TripleDES:
		...
		private static string encrypt(string valueToEncrypt)
		{
			...
			if (!string.IsNullOrEmpty(valueToEncrypt)) {
				...
				try
				{
					...
					CryptoStream mCryptoStream = new CryptoStream(mMemoryStream, mCryptoProvider.CreateEncryptor(KEY_64, IV_64), CryptoStreamMode.Write);
					...
					mRetVal = Convert.ToBase64String(mMemoryStream.GetBuffer(), 0, int.Parse(mMemoryStream.Length.ToString()));
				}
				catch (Exception)
				{
					throw;
			...
			if (!string.IsNullOrEmpty(EncryptedValue)) {
				...
				try
				{
					if (isBase64String(EncryptedValue))
					{
						...
						CryptoStream cs = new CryptoStream(mMemoryStream, mCryptoProvider.CreateDecryptor(KEY_64, IV_64), CryptoStreamMode.Read);
		...
		private static string encryptTripleDES(string valueToEncrypt)
		{
			...
			if (!string.IsNullOrEmpty(valueToEncrypt)) {
				...
				try
				{
					...
					mMemoryStream = new MemoryStream();
					CryptoStream mCryptoStream = new CryptoStream(mMemoryStream, mCryptoProvider.CreateEncryptor(KEY_192, IV_192), CryptoStreamMode.Write);
					StreamWriter mStreamWriter = new StreamWriter(mCryptoStream);
					mStreamWriter.Write(valueToEncrypt);
					mStreamWriter.Flush();
					mCryptoStream.FlushFinalBlock();
					mMemoryStream.Flush();
					//convert back to a string
					retVal = Convert.ToBase64String(mMemoryStream.GetBuffer(), 0, int.Parse(mMemoryStream.Length.ToString()));
				}
				catch (Exception)
				{
					throw;
			...
			if (!string.IsNullOrEmpty(encryptedValue))
			{
				try
				{
					if (isBase64String(encryptedValue))
					{
						...
						byte[] buffer = Convert.FromBase64String(encryptedValue);
						mMemoryStream = new MemoryStream(buffer);
						CryptoStream mCryptoStream = new CryptoStream(mMemoryStream, mCryptoProvider.CreateDecryptor(KEY_192, IV_192), CryptoStreamMode.Read);
