    ...
    public async Task<ActionResult<Tuple<string[], string[]>>> GetFeedbackAccounts()
    {
        ...
        if (mSecurityInfo.MayEdit)
        {
            ...
            if (mDeveloper != null)
            {
                ...
                UIRole mDeveloper_UIRole = await RoleUtility.GetUIProfile(mDeveloper.Id, mSecurityId);
                mAccountsInRole_Developers = mDeveloper_UIRole.AccountsInRole;
            }
            if (mQA != null)
            {
                UIRole mQA_UIRole = await RoleUtility.GetUIProfile(mQA.Id, mSecurityId);
                mAccountsInRole_QA = mQA_UIRole.AccountsInRole;
            }
