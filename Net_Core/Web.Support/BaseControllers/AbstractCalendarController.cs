...
public abstract class AbstractCalendarController : ControllerBase
{
    ...
    {
        MCalendarEvent mCalendarEvent = await CalendarUtility.GetEvent(await SecurityEntityUtility.CurrentProfile(), calendarEventSeqId);
        if (await getEventSecurity(mCalendarEvent))
        {
            ...
            if (mFunctionProfile != null)
            {
                ...
                {
                    return Ok(await CalendarUtility.DeleteEvent(await SecurityEntityUtility.CurrentProfile(), calendarEventSeqId));
                }
            }
        }
    ...
    public async Task<ActionResult<MCalendarEvent>> GetEvent(string action, int id)
    {
        ...
        if (mFunctionProfile != null)
        {
            ...
            if (mSecurityInfo.MayView)
            {
                MCalendarEvent mRetVal = await CalendarUtility.GetEvent(await SecurityEntityUtility.CurrentProfile(), id);
    ...
    public async Task<ActionResult<List<MCalendarEvent>>> GetEvents(string action, string startDate, string endDate)
    {
        ...
        if (mFunctionProfile != null)
        {
            ...
            if (mSecurityInfo.MayView)
            {
                ...
                List<MCalendarEvent> mRetVal = await CalendarUtility.GetEvents(await SecurityEntityUtility.CurrentProfile(), mFunctionProfile.Id, mStartDate, mEndtDate);
                return Ok(mRetVal);
            }
            return StatusCode(StatusCodes.Status401Unauthorized, "The requesting account does not have the correct permissions");
        }
    ...
    public async Task<ActionResult<MCalendarEvent>> SaveEvent(UISaveEventParameters parameters)
    {
        ...
        MCalendarEvent mCalendarEvent = await CalendarUtility.GetEvent(await SecurityEntityUtility.CurrentProfile(), mParameters.calendarEvent.Id);
        if (await getEventSecurity(mCalendarEvent, mParameters.action) && mParameters.calendarEvent.AddedBy == mAccountProfile.Id)
        {
            ...
            MCalendarEvent mRetVal = await CalendarUtility.SaveCalendarEvent(await SecurityEntityUtility.CurrentProfile(), mFunctionProfile.Id, mParameters.calendarEvent);
            return Ok(mRetVal);
        }
        return StatusCode(StatusCodes.Status401Unauthorized, "The requesting account does not have the correct permissions");
    }
    ...
    private async Task<bool> getEventSecurity(MCalendarEvent calendarEvent, string action = null)
    {
        ...
        if (mSecurityEntity != null)
        {
            ...
            {
                MAccountProfile mAccountProfile = await AccountUtility.CurrentProfile();
                if (mAccountProfile.Id == calendarEvent.AddedBy)
                {
                    mRetVal = true;
                }
            }
